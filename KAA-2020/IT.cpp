...
namespace IT
{
	...
	char* IdTable::GetForName(char* id,Flag_type_variable Type)
	{
		...
		_itoa_s(this->for_count, buffer, 10);
		strcat_s(buffer, 255, id);
		switch (Type.type)
		{
		case INT:
			strcat_s(buffer, 255, "_int"); break;
		case UINT:
			strcat_s(buffer, 255, "_uint"); break;
		case CHAR:
			strcat_s(buffer, 255, "_char"); break;
		case BOOL:
			strcat_s(buffer, 255, "_bool"); break;
		case STR:
			strcat_s(buffer, 255, "_str"); break;
		}
		strcat_s(buffer, 255, "_for");
